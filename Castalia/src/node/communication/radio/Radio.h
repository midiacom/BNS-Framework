...
// ECR => Structure for control of Transmission time in the last seconds
typedef struct TxTimeControl_stats {
	double TimeStamp;
	double TxTimeOfPacket;
}TxTimeControl_Type;
...
class Radio: public CastaliaModule {
 private:
	...
	std::list<TxTimeControl_Type> TxTimeControl; // ECR => List for save the Txs in the last seconds.
 ...
 public:
	...
	double getTxTimeInLastSlotTime( double ); // Add by me ECR
};
