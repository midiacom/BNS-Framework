...
// ECR => ********************************************************************************************
    TxTimeControl_Type TxTimeControl_Var;
    TxTimeControl_Var.TimeStamp = simTime().dbl();
    TxTimeControl_Var.TxTimeOfPacket = txTime;
    TxTimeControl.push_back(TxTimeControl_Var);

    for(std::list<TxTimeControl_Type>::iterator i=TxTimeControl.begin(); i!=TxTimeControl.end(); ++i){
        TxTimeControl_Type temp_var = *i;

        if((TxTimeControl_Var.TimeStamp - temp_var.TimeStamp) >= 1){ //ECR => Just leaves transmissions made in the last second. 
           TxTimeControl.pop_front();
		   i=TxTimeControl.begin();
        } else{
            break;
        }
...
// ECR *************************************************************************************************
double Radio::getTxTimeInLastSlotTime(double SlotTime) 
{
	...
	for(std::list<TxTimeControl_Type>::iterator i=TxTimeControl.begin(); i!=TxTimeControl.end(); ++i){
		...
		TxTimeControl_Type Temporal_Var = *i;

		if((time_var - Temporal_Var.TimeStamp) <= SlotTime){ //ECR => Just leaves transmissions made in the last second. 
			TxTime_sum += Temporal_Var.TxTimeOfPacket;
		}
 	}
