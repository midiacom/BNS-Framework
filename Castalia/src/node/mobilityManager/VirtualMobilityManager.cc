...
void VirtualMobilityManager::parseDeployment() {
	...
	nodeLocation.nodePlacement = node->par("nodePlacement");
...
void VirtualMobilityManager::setLocation(double x, double y, double z, double phi, double theta, int nodePlacement)
{
	nodeLocation.x = x;
	nodeLocation.y = y;
	nodeLocation.z = z;
	...
	nodeLocation.nodePlacement = nodePlacement;
...
void VirtualMobilityManager::notifyWirelessChannel()
{
	...
	positionUpdateMsg->setNodePlacement(nodeLocation.nodePlacement);
